use std::str;
use std::collections::BTreeMap;
#[derive(Debug)]
struct PokerHand<'a> {
    card: &'a str,
    rank: u32,
}
impl<'a> PokerHand<'a> {
    fn new(card: &'a str) -> Self {
        PokerHand {card, rank: PokerHand::rank(card)}
    }
    fn rank(card: &str) -> u32{
        const STRAIGHT_FLUSH: u32 = 20_00_00_90_00;
        const FOUR_KIND: u32 = 20_00_00_80_00;
        const FULL_HOUSE: u32 = 20_00_00_70_00;
        const FLUSH: u32 = 20_00_00_50_00;
        const STRAIGHT: u32 = 20_00_00_40_00;
        const THREE_KIND: u32 = 20_00_00_30_00;
        const TWO_PAIRS: u32 = 20_00_00_20_00;
        const ONE_PAIR: u32 = 20_00_00_10_00;
        
        let suits = card.split(' ').map( |chars| &chars[chars.len()-1..] ).collect::<Vec<_>>();
        let kinds = card.split(' ')
        .map( |chars| match &chars[..chars.len()-1] {
            "J" => 11,
            "Q" => 12,
            "K" => 13,
            "A" => 14,
            n => n.parse().unwrap()
        })
        .fold(BTreeMap::<u32, u32>::new(), |mut dict, rank| {
            *dict.entry(rank).or_insert(0) += 1;
            dict
        });
        let mut kind_and_count = kinds.iter().map(|(&x,&y)| (x,y)).collect::<Vec<_>>();
        kind_and_count.sort_by( |a,b| b.1.cmp(&a.1) );
        match kind_and_count.as_slice() {
            [(a,4),(b,1)] => FOUR_KIND + 10*a + b,
            [(a,3),(b,2)] => FULL_HOUSE + 10*a + b,
            [(a,3), (b,1),(c,1)] => THREE_KIND + 10*a + b + c,
            [(a,2),(b,2),(c,1)] => TWO_PAIRS + 10*a + 10*b + c,
            [(a,2),(b,1),(c,1),(d,1)] => ONE_PAIR + 10*a + b + c + d,    
            _ => {
                let kind_values = kinds.keys().cloned().collect::<Vec<_>>();
                let is_same_suit = suits.windows(2).all( |p| p[0] == p[1] );
                let is_asc_order = kind_values.windows(2).all( |d| d[0]+1 == d[1] || d[1] == 14);
                let fake_last_ace = kind_values[4] == 14 && kind_values[3] != 13;
                let sum: u32 = kind_values.iter().sum();
                match (is_same_suit, is_asc_order, fake_last_ace) {
                    (true, true, false) => STRAIGHT_FLUSH + sum,
                    (true, true, true) => STRAIGHT_FLUSH + sum - 14,
                    (true, false, _) => FLUSH + sum,
                    (false, true, false) => STRAIGHT + sum,
                    (false, true, true) => STRAIGHT + sum - 14,
                    (false, false, _) => {
                        let s = kind_values.iter().rev().map( |d| d.to_string()).collect::<String>();
                        s.parse().unwrap()
                    }
                }
            }
        }
    }
}

pub fn winning_hands<'a>(hands: &[&'a str]) -> Vec<&'a str> {
    if hands.len() == 0 {
        vec![]
    }
    else {
        let mut poker_hands = hands.iter().map( |h| PokerHand::new(h) ).collect::<Vec<_>>();
        poker_hands.sort_by( |h1, h2| h2.rank.cmp(&h1.rank) );
        poker_hands.iter().filter( |d| d.rank == poker_hands[0].rank).map(|d| d.card).collect()
    }
}


fn main() {
    let poker = &["3S 3S 3S 3S 9H", "3H 4H 5C 6C JD", "3H 3H 3H 3H 10D"];
    let result = winning_hands(poker);
    println!("{result:?}");
}
